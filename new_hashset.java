...
class MathProblemGeneratorGUI_hashset extends JFrame {
    ...
    private final HashSet<Problem> problems = new HashSet<>();
    private final ArrayList<Problem> problemsList = new ArrayList<>();
    ...
    public MathProblemGeneratorGUI_hashset() {
        initializeUI();
    }

    private void initializeUI() {
        setTitle("Math Problem Generator (HashSet)");
        ...
        questionList.addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (currentIndex >= 0 && currentIndex < problemsList.size()) {
    ...
    private void generateProblems(ActionEvent e) {
        ...
        while (problems.size() < needed && attempts < maxAttempts) {
            ...
            if (!failed && opsDone > 0) {
                ...
                Problem prob = new Problem(questionString, result);
                if (problems.add(prob)) { // only add if unique
    ...
    private void showSelectedQuestion() {
        if (currentIndex >= 0 && currentIndex < problemsList.size()) {
            Problem p = problemsList.get(currentIndex);
            progressLabel.setText("Question " + (currentIndex + 1) + " / " + problemsList.size());
    ...
    private void nextQuestion(ActionEvent e) {
        if (currentIndex >= 0 && currentIndex < problemsList.size()) {
            Problem current = problemsList.get(currentIndex);
            ...
            if (currentIndex < problemsList.size() - 1) {
                currentIndex++;
    ...
    private void showResults() {
        ...
        sb.append(String.format("Final Score: %d / %d%n%n", score, problemsList.size()));
    ...
    private static class Problem {
        ...
        public boolean equals(Object obj) {
            ...
            return Objects.equals(questionString, other.questionString) && answer == other.answer;
        }

        @Override
        public int hashCode() {
            return Objects.hash(questionString, answer);
        }
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            MathProblemGeneratorGUI_hashset gui = new MathProblemGeneratorGUI_hashset();
            gui.setVisible(true);
        });
    }
}